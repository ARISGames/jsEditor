    ...
    <select class="form-control link-id">
      ...
      <% if(link_scripts) { %>
        <% scripts.each(function(script) { %>
          <option value="<%= script.id %>" <%= option_selected(link_id === script.id) %>><%= _.str.prune(speakerfromscriptid(script.id)+": "+escaped_prompt(strip_js(script.get("text")))+" ("+script.id+")", 600) %></option>
        <% }); %>
