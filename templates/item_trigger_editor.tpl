<% if(!in_modal) { %>
	<h4>Item Trigger <span class="object-id text-muted"><%= is_new ? "" : item_id %></span></h4>
	<h5><%= name %></h5>
<% } %>

<form class="form" role="form" onsubmit="return false;">
<% if(visible_fields === "create_item_with_trigger" ) { %>

<!-- Item attributes -->

<div class="form-group">
	<label class="sr-only" for="item-name">Item Name</label>
	<input type="text" autofocus class="form-control" id="item-name" placeholder="Enter Name" value="<%= name %>">
</div>
