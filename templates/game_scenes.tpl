<button type="button" class="new-scene btn btn-default">
	<span class="glyphicon glyphicon-plus-sign"></span>
	Add Scene
</button>
<div class="scenes">
	<div class="scene">
		<h4>Exhibit Room</h4>
		<ul class="list-inline scene-items">
			<li class="scene-item link-start">
				<div class="item-icon"><span class="glyphicon glyphicon-user"></span></div>
				<p class="item-label">Zero</p>
			</li>
			<li class="scene-item link-end">
				<span class="glyphicon glyphicon-user"></span>
				<p class="item-label">Mizu</p>
			</li>
			<li class="scene-item link-end">
				<span class="glyphicon glyphicon-film"></span>
				<p class="item-label">Dino Clip</p>
			</li>
			<div class="clear"></div>
		</ul>
	</div>

	<div class="scene">
		<h4>Lake Side</h4>
		<ul class="list-inline scene-items">
			<li class="scene-item link-end">
				<span class="glyphicon glyphicon-user"></span>
				<p class="item-label">Zero</p>
			</li>
			<li class="scene-item">
				<span class="glyphicon glyphicon-film"></span>
				<p class="item-label">Clue Video</p>
			</li>
			<li class="scene-item">
				<span class="glyphicon glyphicon-tag"></span>
				<p class="item-label">Clue Note</p>
			</li>
			<div class="clear"></div>
		</ul>
	</div>

	<div class="scene">
		<h4>Outer Space</h4>
		<ul class="list-inline scene-items">
			<li class="scene-item link-start">
				<span class="glyphicon glyphicon-user"></span>
				<p class="item-label">Pirate</p>
			</li>
			<li class="scene-item link-start">
				<span class="glyphicon glyphicon-qrcode"></span>
				<p class="item-label">Web Link</p>
			</li>
			<div class="clear"></div>
		</ul>
	</div>
</div>

<svg class="scenes-overlay" preserveAspectRatio="xMinYMin meet">
  <path class="link-line" d="M0 0 L10 10" stroke="rgba(200, 0, 200, 0.75)" stroke-dasharray="3,3" fill="none" stroke-width="2.5" />
  <path class="link-line" d="M0 0 L10 20" stroke="rgba(200, 0, 200, 0.75)" stroke-dasharray="3,3" fill="none" stroke-width="2.5" />
  <path class="link-line" d="M0 0 L10 30" stroke="rgba(200, 0, 200, 0.75)" stroke-dasharray="3,3" fill="none" stroke-width="2.5" />
</svg>
