    ...
    sync: function(method, model, options)
    {
      ...
      else if(method === "update" || method === "create" || method === "delete")
      {
        ...
        $.each(this.get_amfphp_url_attributes(), function(index, key)
        {
          ...
          if(key === model.idAttribute && model.get(key) == null) return;
          ...
          else throw "syncError during "+method+": attribute '"+key+"' not found on model";
        });
      ...
      options.success = function(data, success, success_options)
      {
        ...
        if(data.faultCode) throw "amf Model Fault: "+data.faultString+" for "+options.url;
